...
import {animateElement,animateElementTillComplete} from '../../../libs/animation'
import {madFaceAnimation, sadFaceAnimation} from '../../../libs/svgAnimation'
...
export class MadButton extends React.Component {
  ...
  componentDidMount(){
    madFaceAnimation(this.madFace);
  }
  componentWillLeave (callback) {
    const svgButton = this.svgButton;
    animateElement(svgButton,'750','translateY',["0","-20px"],'easeInOutQuint');
    animateElementTillComplete(svgButton,'750','opacity',[1,0],'easeInOutQuint').then(()=>{
      // state = complete
      this.props.updateMood(true);
      callback();
    });
  }
  componentWillEnter (callback) {
    const svgButton = this.svgButton;
    animateElement(svgButton,'750','translateY',["-20px", "0"],'easeInOutQuint');
    animateElementTillComplete(svgButton,'750','opacity',[0,1],'easeInOutQuint').then(()=>{
      callback();
    });
  }
  render(){
    return (
      <SVGButton ref={svgButton => this.svgButton = svgButton}>
        <svg
          ...
          <g id="madFace" ref={madFace => this.madFace = madFace}>
            <path
              id="mouth"
...
export class SadButton extends React.Component {
  componentDidMount(){
    sadFaceAnimation(this.faceInside,this.rightEye,this.leftEye,this.leftBrow,this.rightBrow,this.bigTear);
  }
  componentWillLeave (callback) {
    const svgButton = this.svgButton;
    animateElement(svgButton,'750','translateY',["0","-20px"],'easeInOutQuint');
    ...
    return (
      <SVGButton ref={svgButton => this.svgButton = svgButton}>
        <svg id="sad" data-name="Sad" xmlns="http://www.w3.org/2000/svg"viewBox="0 0 544 544">
          ...
          <g id="faceInside" ref={faceInside => this.faceInside = faceInside}>
            <ellipse id="leftEye" ref={leftEye => this.leftEye = leftEye} cx="199" cy="205" rx="48" ry="61"/>
            <ellipse id="rightEye" ref={rightEye => this.rightEye = rightEye} cx="353" cy="205" rx="48" ry="61"/>
            <path id="mouth" ref={mouth => this.mouth = mouth} d="M1010.52,670.72c25.94,0,49.47,8.6,66.69,22.56,5.7,4.62,13.45-2.36,9.36-8.45a91.91,91.91,0,0,0-153.35,1.07c-4,6.14,3.85,13,9.48,8.3C960,679.7,984,670.72,1010.52,670.72Z" transform="translate(-735 -305)" fill="#2c210b"/>
            <path id="leftBrow" ref={leftBrow => this.leftBrow = leftBrow} d="M928.6,401.86h0a10.14,10.14,0,0,1,1.71-.46c8.48-1.78,36.53,5.94,19.37,20l-.76.39-.63.24c-33.35,13.9-73.58,31.19-86.54,37.28a21.74,21.74,0,0,1-5.45,1.71,13.55,13.55,0,0,1-8.33-1c-3.12-1.51-6.25-4.65-3.89-11a5.74,5.74,0,0,1,1.81-2.46l4.45-3.16C867,432.33,900.57,413.8,928.6,401.86Z" transform="translate(-735 -305)" fill="#71583d"/>
            <path id="rightBrow" ref={rightBrow => this.rightBrow = rightBrow} d="M1092.76,400.61h0a8.8,8.8,0,0,0-1.7-.46c-8.48-1.79-36.53,5.94-19.37,20l.75.39.63.24c33.36,13.9,73.59,31.19,86.55,37.28a21.74,21.74,0,0,0,5.45,1.71,13.62,13.62,0,0,0,8.33-1c3.12-1.52,6.25-4.65,3.89-11a5.71,5.71,0,0,0-1.81-2.45L1171,442.1C1154.32,431.08,1120.8,412.54,1092.76,400.61Z" transform="translate(-735 -305)" fill="#71583d"/>
            <path id="bigTear" ref={bigTear => this.bigTear = bigTear} d="M1188.3,696.57c-.55-.39-10.33.28-19.12-1s-16.55-4.61-17-5c-20.89-18.26-10.87-52.84-9-58.71.17-.5.35-1,.56-1.48l44.2-101.37h0l44.2,101.37c.21.49.39,1,.56,1.48,1.93,6,12.32,41.84-10.25,59.78-7.69,4-16.35,3.75-17.6,4.23Z" transform="translate(-735 -305)" fill="url(#linear-gradient-2)"/>
          </g>
        </svg>
        <span>GET SAD</span>
      </SVGButton>   
...
export class NextButton extends React.Component {
  componentWillLeave (callback) {
    ...
    animateElement(nextButton,'750','translateY',["0","-20px"],'easeInOutQuint');
    animateElementTillComplete(nextButton,'750','opacity',[1,0],'easeInOutQuint').then(()=>{
      callback();
    });
  }
  componentWillEnter (callback) {
    const nextButton = this.nextButton;
    animateElement(nextButton,'750','translateY',["-20px", "0"],'easeInOutQuint');
    animateElementTillComplete(nextButton,'750','opacity',[0,1],'easeInOutQuint').then(()=>{
      callback();
    });
  }
  render(){
